...
int main(){
printf("EAS Cluster 'Worker' 7.5.f\n");
//test Area 
int Start[24]={46,46,46,46,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100};
int numbers=0;
int alphaENG=1;
int symbols=0;
int times=1000000;
...
int i,j,Line_MAX_changer,FLAG_Check,end_of_string,flag,max,eodigits;
char Newline[24];
FILE *fp;
...
printf("Stage 3: Calculating and Generating the lst...\n");
 for(end_of_string=0;end_of_string<24;end_of_string++){
   if(start[end_of_string]==100){ //100 means there is \0 in the string version
     j = end_of_string;break;
   }
  }
 for(i=0;i<times;i++){
  ...
  start[j-1]++; //incresing the int Array by one
   ...
   for(Line_MAX_changer=j-1;Line_MAX_changer>0;Line_MAX_changer--){
   if(start[Line_MAX_changer]==max+1){
     start[Line_MAX_changer]=0;
     start[Line_MAX_changer-1]++;
     break;
   }
   //flag to check if all the possible compinations have been tested and add another line
    for(FLAG_Check=j-1;FLAG_Check>=0;FLAG_Check--){
      if(start[FLAG_Check]==max){
        flag=0;
      }else{
        flag=1;break;
      }
